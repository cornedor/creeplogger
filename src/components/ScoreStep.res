  ...
  let saveGame = async () => {
    ...
    let _ = await Promise.all(
      Array.map(bluePlayers, async player => {
        Players.updateOpenSkillGameStats(player.key, blueState, redState, Blue, player.mu, player.sigma, player.ordinal)
      }),
    )
    let _ = await Promise.all(
      Array.map(redPlayers, async player => {
        Players.updateOpenSkillGameStats(player.key, redState, blueState, Red, player.mu, player.sigma, player.ordinal)
      }),
    )
