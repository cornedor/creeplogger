...
async function sendFifaUpdate(allPlayers, playerScores) {
  var formatHandleOrName = function (player) {
    var handle = player.mattermostHandle;
    if (handle !== undefined) {
      return "@" + handle;
  ...
  var playerLines = allPlayers.map(function (player) {
          var score = Core__Option.getOr(Js_dict.get(playerScores, player.key), 0);
          var eloChange = player.fifaLastEloChange;
          var sign = eloChange >= 0.0 ? "+" : "";
          var roundedEloChange = Elo.roundScore(eloChange);
          return "| " + formatHandleOrName(player) + " | " + score.toString() + " | " + sign + roundedEloChange.toString() + " |";
        }).join("\n");
